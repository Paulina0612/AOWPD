#include <iostream>
using namespace std;


// Function to get the maximum value in the array
long int GetMax(int n, long int* table) {
    long int max = table[0];
    for (int i = 1; i < n; i++) {
        if (table[i] > max) {
            max = table[i];
        }
    }
    return max;
}


long int* CountingSort(int n, long int* table, long int exp) {
    
    // Output array that will hold the sorted numbers
    long int *sortedArray;
    sortedArray = (long int*)malloc(n * sizeof(long int));

    if (sortedArray == NULL) {
        cerr << "Nie udalo sie zaalokac pamieci!" << endl;
        exit(1);
    }

    // Initialize count array
    int i, countArray[10] = { 0 };

    // Store count of occurrences in countArray[]
    for (i = 0; i < n; i++) {
        countArray[(table[i] / exp) % 10]++;
    }

    // Change countArray[i] so that it contains actual position of this digit in sortedArray[]
    for (i = 1; i < 10; i++)
        countArray[i] += countArray[i - 1];

    // Build the output array
    for (i = n - 1; i >= 0; i--) {
        sortedArray[countArray[(table[i] / exp) % 10] - 1] = table[i];
        countArray[(table[i] / exp) % 10]--;
    }

    return sortedArray;
}


void CPURadixSort(int n, long int* table) {
    // Find the maximum number to know the number of digits
    long int max = GetMax(n, table);

    // Radix sort loop
    for (long int exp = 1; max / exp > 0; exp *= 10) {
        table = CountingSort(n, table, exp);
    }
}